...
@AndroidEntryPoint
class UpdateProfilePicFragment : Fragment(R.layout.fragment_update_profile_pic) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        slideUpViews(requireContext(),textView11,iv_update_profile_pic,btn_profile_update)
        viewmodal = ViewModelProvider(requireActivity()).get(MainViewModal::class.java)
        subscribeToObserver()
        iv_update_profile_pic.setOnClickListener {
    ...
    fun subscribeToObserver(){
        ...
        viewmodal.imageStatus.observe(viewLifecycleOwner){
            ...
            imageuri=it
            glide.load(imageuri).into(iv_update_profile_pic)
        }

        viewmodal.profileImageStatus.observe(viewLifecycleOwner, EventObserver(

            onError = {
                pb_update_profile.isVisible=false
                snackbar(it)
            },
            onLoading = {
                pb_update_profile.isVisible=true
            }
        ...
        viewmodal.profileImageStatus2.observe(viewLifecycleOwner,EventObserver(
